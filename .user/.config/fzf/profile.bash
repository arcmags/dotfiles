#!/bin/bash
## ~/.config/fzf/profile.bash ::
# default: /usr/share/fzf/key-bindings.bash
...
__fzf_select__() {
    local cmd opts
    cmd="${FZF_CTRL_T_COMMAND:-"command find -L . -mindepth 1 \
      \\( -path '*/\\.*' -o -fstype 'sysfs' -o -fstype 'devfs' -o -fstype 'devtmpfs' \
      -o -fstype 'proc' \\) -prune -o -type f -print -o -type d -print -o -type l \
...
__fzf_history__() {
    ...
    output=$(builtin fc -lnr -2147483648 | \
      last_hist=$(HISTTIMEFORMAT='' builtin history 1) perl -n -l0 -e "$script" | \
      FZF_DEFAULT_OPTS="$opts" $(__fzfcmd) --query "$READLINE_LINE") || return
