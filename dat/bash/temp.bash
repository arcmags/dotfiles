...
# args:
opts_long=('remote:' 'verbose' 'help' 'nocolor' 'quiet')
opts_short='r:VHQ'
...
parse_args() {
    args_parsed=()
    local a=0 i=0 arg= sflgs= sopts= sflgopts= sflgopts="$1"
    ...
    while [ -n "$arg" ]; do
        case "$arg" in
            ...
            --*) if [[ " ${lflgs[*]} " =~ " ${arg:2} " ]]; then
                    ...
                    [ $n_args -le $((a+1)) ] && error_optarg "$arg"
                    args_parsed+=("$arg" "${args[((++a))]}")
                else
                    break
                fi
